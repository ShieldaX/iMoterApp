...
local function isMultiTouchable()
  if not system.hasEventSource( "multitouch" ) then
--    local msg = display.newText( "Multitouch events not supported on this platform", 0, 20, native.systemFontBold, 13 )
--    msg.x = display.contentWidth / 2
--    msg:setFillColor( 1, 0, 0 )
...
-- ----------------------------------------
-- ----------------------------------------
-- Pinch Zoom Gesture Class as [Mixin Class]
-- TargetClass:include(PinchZoomGesture)
--
local PinchZoomGesture = class "PinchZoomGesture"

function PinchZoomGesture:initialize()
  if isMultiTouchable() then
    d('Multitouch Actived -- Pinch and zoom the image')
  else
    d('Multitouch events not supported on this platform')
  end
end

function PinchZoomGesture:touch(event)
  local result = true
...
function PinchZoomGesture:activeOn(name)
  if self[name] and util.isDisplayObject(self[name]) then
    self[name]:addEventListener('touch', self)
    return true
  else
    return false
  end
...
function PinchZoomGesture:onInclude()
  d('Multitouch Gesture Included')
end
